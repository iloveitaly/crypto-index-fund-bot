...
Here are some features I _didn't_ want that I could imagine others would like:
...
Right now, there are some variables that are hardcoded into the `User` class that you may want to change. Assuming you've taken a look at the `User` options _and_ configured `.env` you can run `python main.py --help` to get the following information:
...
1. Get a Coin Market Cap api token - https://coinmarketcap.com/api/

2. Generate python encryption key

`python manage.py generate_encryption_key`

3. Set the heroku config

```
heroku buildpacks:add heroku/python
heroku buildpacks:add https://github.com/moneymeets/python-poetry-buildpack.git
heroku config:set PYTHON_RUNTIME_VERSION=3.9.7
heroku config:set DJANGO_SECRET_KEY=any_secure_long_string
heroku config:set USER_BINANCE_API_KEY=xxxxx
heroku config:set USER_BINANCE_SECRET_KEY=yyyyy
heroku config:set DJANGO_FIELD_ENCRYPTION_KEY=zzzzz
heroku config:set REDIS_URL=none
heroku config:set TEST_DATABASE_URL=none
heroku config:set DISABLE_COLLECTSTATIC=1
heroku config:set COINMARKETCAP_API_KEY=aaaaaa
heroku config:set USER_LIVEMODE=true
```
...
The bot will _not_ submit an order for a token that has an existing open order.
